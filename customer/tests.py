...
class LendingApiTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.customer2 = User.objects.create_user("customer2@gmail.com", "Xcaioshg99!")
        self.customer3 = User.objects.create_user("customer3@gmail.com", "Xcaioshg99!")
        self.customer_profile = Customer.objects.create(full_name="test customer", user=self.customer)
        self.mobile_wallet = MobileWallet.objects.create(phone_number="254768554234", customer=self.customer_profile)
        ...
        self.loan_product = LoanProduct.objects.create(**self.product_a_payload)
    ...
    def test_create_and_update_customer_profile(self):
        ...
        response = self.client.put(reverse("get-update-customer", kwargs={"id": data["id"]}),
                                   data={"full_name": full_name})
        data = response.json()
    ...
    def test_create_and_update_mobile_wallet(self):
        # Create profile
        Customer.objects.create(full_name="test customer 2", user=self.customer2)
        ...
        response = self.client.post(reverse("mobile_wallet"),
        ...
        response = self.client.put(reverse("get-update-mobile_wallet", kwargs={"id": data["id"]}),
                                   data={"phone_number": phone_number})
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data.get("phone_number"), phone_number)
    ...
    def test_create_loan_offer(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(data.get("id"))
        self.assertEqual(data.get("due_date"),
                         (datetime.date.today() + datetime.timedelta(self.loan_product.duration)).strftime("%Y-%m-%d"))
    ...
    def test_lending_flow(self):
        ...
        product_a = self.client.post(reverse("loan_product"),
                                     data=self.product_a_payload).json()
        product_b = self.client.post(reverse("loan_product"),
                                     data=self.product_b_payload).json()
        ...
        self.client.post(reverse("customer"),
                         data={"full_name": full_name})

        # create wallet
        wallet_data = self.client.post(reverse("mobile_wallet"),
                                       data={"phone_number": "254729445215"}).json()
        ...
        response = self.client.get(reverse("get-update-mobile_wallet", kwargs={"id": wallet_data["id"]}))
        data = response.json()
        wallet_amount = data["amount"]
        self.assertEqual(loan_offer_amount, wallet_amount)

        # pay loan manually
        response = self.client.post(reverse("pay_loan"),
